...
import { DAI, popupCenter, USDC, USDT, WETH } from 'src/app/shared/utils/utils';
...
export class PairExplorerComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.interval = setInterval(() => {
      this.mostRecentTimestamp = this.listOfData && this.listOfData.length && this.listOfData[0].date;
    ...
    const paramsSub = this.route.params.subscribe((params) => {
      ...
      this.tokenId = params.tokenId && (params.tokenId as string).toLocaleLowerCase();
      this.geckoInfo = null;
        ...
        this.that.barsSub = barsQuery.valueChanges.subscribe(
          (result) => {
            ...
            const tokenTrades = [...trades].filter(
              (item) =>
                item &&
                item.baseCurrency.address.toLowerCase() !== '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
            );
    ...
    const swapsSub = this.query.valueChanges.subscribe(
      async (result) => {
        ...
        this.pairUnit = this.tokenIsFirstInPair
          ? result.data.pair.token1
          : result.data.pair.token0;
        const formattedSwap = this.buildSwaps(result)
          .sort((a, b) =>
            b.date - a.date ||
        ...
        if (this.counter === 1) {
          ...
          const chartIsReadySub = this.tokenService.chartIsReady
            .subscribe((isReady) => {
              if (isReady) {
                setTimeout(() => {
                  const mostRecentSwap = formattedSwap[formattedSwap.length - 1];
          ...
          const subGecko = this.tokenService
            ...
            .subscribe(
              ...
              ({ error }) => {
                console.log('getTokenGeckoInfo: ', error);
                this.geckoInfo = null;
              }
            );
  ...
  getDailyVolume(pairHourData: any[]) {
    ...
    const multiplicator = this.pairUnit.symbol === 'WETH'
      ? this.tokenService.ethPrice
      : 1;

    return pairHourData.reduce((res, curr) => {
      return (
        res +
        +(this.tokenIsFirstInPair
  ...
  savePriceAlert(val: number | string) {
