...
export class PancakePairExplorerComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    const paramsSub = this.route.params.subscribe((params) => {
      this.pairId = params.tokenId && (params.tokenId as string).toLocaleLowerCase();
      this.geckoInfo = null;
      ...
      const addressesSub = queryAddresses.valueChanges.subscribe(
        ({ data }: any) => {
          this.tokenService.setTokenId(data.ethereum.dexTrades[0].buyCurrency);
          this.tokenService.setTokenId(
    ...
    const swapsSub = this.query.valueChanges.subscribe((res) => {
      ...
      const pairInfoSub = queryPairInfo.valueChanges.subscribe((res2) => {
        ...
        this.pairInfo = {
          ...this.pairInfo,
          id: this.pairId,
          priceInUSD: this.listOfDisplayData && this.listOfDisplayData[0].priceUSD,
          priceInQuote: this.listOfDisplayData && this.listOfDisplayData[0].priceETH,
  ...
  // getCandlesData() {
  //   const objArray = [...this.listOfData].map((d) => ({
  ...
  //     )
  //   );
  //   return groupedAndReduced.sort((a, b) => a.time - b.time);
  // }
  ...
  //   const timeToReturn = this.formatDate(time);
  ...
  removeDuplicateSwaps(myArr) {
    return myArr
      .filter((obj, pos, arr) => arr.map((mapObj) => mapObj.txId).indexOf(obj.txId) === pos)
